...
import com.lin.spi.exception.SPIRuntimeException;
import com.lin.spi.util.ClassLoaderUtils;
import com.lin.spi.util.ClassTypeUtils;
import com.lin.spi.util.ClassUtils;
import com.lin.spi.util.StringUtils;
...
public class ExtensionLoader<T> {
    ...
    protected ExtensionLoader(Class<T> interfaceClass, boolean autoLoad, ExtensionLoaderListener<T> listener) {
        ...
        if (autoLoad) {
            List<String> paths = SPIConfig.getInitSetValue(SPIConfig.SPIPathKey, Collections.emptyList());
        ...
        try {
            ...
            if (LOGGER.isWarnEnabled()) {
                LOGGER.warn("Extension {} of extensible {} is disabled, cause by: {}",
                        className, interfaceName, e.getMessage());
            }
            return;
        }
        ...
        if (extensionClass != null) {
            ...
            for (Map.Entry<String, ExtensionClass<T>> entry : all.entrySet()) {
                ...
                if (extensionClass.getOrder() >= existed.getOrder()) {
                    ...
                    if (Objects.nonNull(rejection) && rejection.length > 0) {
    ...
    public T getExtension(String alias) {
        ...
        if (extensionClass == null) {
            throw new SPIRuntimeException("Not found extension of " + interfaceName + " named: \"" + alias + "\"!");
        } else {
